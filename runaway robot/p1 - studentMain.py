...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if len(OTHER) < 2:
        ...
        DES = [distance_between(OTHER[i], OTHER[i-1]) for i in range(1, len(OTHER))] #Distance each step
        desloc_factor = sum(DES) / len(DES)
        
        AES = [atan2(OTHER[i][1] - OTHER[i-1][1], OTHER[i][0] - OTHER[i-1][0]) for i in range(1, len(OTHER))] #Angle each step
        angle_diff = [AES[i] - AES[i-1] for i in range(1, len(AES))]
        
        turn_factor = sum(angle_diff) / len(angle_diff)
    
        x = measurement[0] + desloc_factor * cos(AES[-1] + turn_factor)
        y = measurement[1] + desloc_factor * sin(AES[-1] + turn_factor)
        return [x, y], OTHER
